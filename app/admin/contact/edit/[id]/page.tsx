...
export default function EditContactLink({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    // Load contact link data
    const contactLinks = JSON.parse(localStorage.getItem('contactLinks') || '[]')
    const contactLink = contactLinks.find((link: ContactLink) => link.id === parseInt(params.id))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Update contact link
    const updatedContactLinks = existingContactLinks.map((link: ContactLink) => 
      link.id === parseInt(params.id) 
        ? {
            ...link,
    ...
    <div className="min-h-screen bg-black text-white relative allow-select">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl font-bold mb-8">编辑联系方式</h1>
